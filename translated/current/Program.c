...
string convert_reference_type__3(BOOL const mutable_binding__, Type__0 const *_Nonnull type__, BOOL const nullable__)
{
	...
	sb_append__2(c_type__, ((string){2,(uint8_t const*)" *"}));
	if (cond(nullable__))
	{
		sb_append__2(c_type__, ((string){9,(uint8_t const*)"_Nullable"}));
	}
	else
	{
		sb_append__2(c_type__, ((string){8,(uint8_t const*)"_Nonnull"}));
	}
...
string convert_type__3(BOOL const mutable_binding__, Type__0 const *_Nonnull type__, BOOL const optional__)
{
	...
	if (cond(bool_op(bool_arg(type__->is_primitive__) && bool_arg(string__0op__equal(unqualified_name__1(type__->name__), ((string){8,(uint8_t const*)"optional"}))))))
	{
		...
		{
			system__text__String_Builder__0 *_Nonnull const c_type__ = system__text__String_Builder__0__0new__1(allocate(sizeof(system__text__String_Builder__0)), ((string){11,(uint8_t const*)"p_optional<"}));
			sb_append__2(c_type__, convert_type__3(TRUE, optional_type__, TRUE));
			sb_append__2(c_type__, ((string){1,(uint8_t const*)">"}));
