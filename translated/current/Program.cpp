...
public:
	...
	auto construct_declares_type(::Type_ const *_Nonnull const type_, ::Syntax_Node_ const *_Nonnull const syntax_, ::System_::Collections_::List_<::Semantic_Node_ const *_Nonnull> const *_Nonnull const children_) -> ::Semantic_Node_*;
...
public:
	...
	auto Emit_() -> str;
private:
	static auto mangle_name_(::Name_ const *_Nonnull const name_) -> str;
...
auto ::Semantic_Node_::construct_declares_type(::Type_ const *_Nonnull const type_, ::Syntax_Node_ const *_Nonnull const syntax_, ::System_::Collections_::List_<::Semantic_Node_ const *_Nonnull> const *_Nonnull const children_) -> ::Semantic_Node_*
{
	::Semantic_Node_* self = this;
...
auto ::Semantic_Tree_Builder_::build_(::Syntax_Node_ const *_Nonnull const syntax_, ::Name_Table_ const *_Nonnull const name_table_, ::Name_Subtable_ const *_Nonnull const scope_) const -> ::Semantic_Node_ const *_Nonnull
{
	...
	else if (cond(bit_op(bit_arg(equal_op(syntax_->kind_, ClassDeclaration_)) || bit_arg(equal_op(syntax_->kind_, StructDeclaration_)))))
	{
		...
		return (new ::Semantic_Node_())->construct_declares_type(type_scope_->type_, syntax_, children_);
	}
...
auto ::Token_Stream_::NewIdentifierOrKeyword_(u32 const end_) -> ::Syntax_Node_ const *_Nonnull
{
	...
	else
	{
		...
		{
			::Text_Span_ const *_Nonnull diagnosticSpan_ = (new ::Text_Span_())->construct(position_, end_.op_subtract(position_));
			diagnostics_->add_((new ::Diagnostic_())->construct(CompilationError_, Lexing_, source_, diagnosticSpan_, str("Identifiers ending with underscore are reserved `").op_add(value_)->op_add(str("`"))));
		}
	}
...
auto ::Emitter_::mangle_name_(::Name_ const *_Nonnull const name_) -> str
{
...
auto ::Emitter_::contains_multi_underscore_runs_(str const value_) -> bit
{
	i32 i_ = i32(0);
...
auto ::Emitter_::emit_member_declaration_(::Semantic_Node_ const *_Nonnull const member_, str const class_name_, bit const is_value_type_, i32 const current_access_level_) -> i32
{
	...
	if (cond(equal_op(member_->kind_, ConstructorDeclaration_)))
	{
		...
		str return_type_ = str("::").op_add(class_name_);
		...
		definitions_->WriteLine_(str("auto ::").op_add(class_name_)->op_add(str("::"))->op_add(full_name_)->op_add(parameters_)->op_add(str(" -> "))->op_add(return_type_));
...
auto ::Emitter_::emit_default_constructor_(str const type_name_, bit const is_value_type_, i32 const current_access_level_) -> void
{
	...
	str return_type_ = str("::").op_add(type_name_);
	if (cond(bit_not(is_value_type_)))
	{
		return_type_ = return_type_.op_add(str("*"));
	}
...
auto ::Emitter_::emit_declaration_(::Semantic_Node_ const *_Nonnull const declaration_) -> void
{
	...
	{
		::Type_ const *_Nonnull const declared_type_ = declaration_->declares_type_;
